...
class _ExerciseGroupCarouselScreenState
  ...
  List<List<UserExerciseRow>> userExerciseRows =
      []; // UserExerciseRow из program_exercise_sets_table.dart
  ...
  Future<void> _loadAllLastResults(List<ExerciseModel> exercises) async {
    for (int i = 0; i < exercises.length; i++) {
      // Загружаем для всех подходов, включая дополнительные
      for (int set = 0; set < userExerciseRows[i].length; set++) {
        await _loadLastUserExerciseResult(i, set, exercises[i].uuid);
      }
    }
  }
